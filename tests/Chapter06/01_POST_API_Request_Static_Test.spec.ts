...
test('POST API Request Test using Static file in Playwright', async ({ request }) => {

    //Using request context, we are calling post method to send a post request to the endpoint. 
    // We need to send the end point (/booking) that gets appended to baseURL and the data which is coming from the static JSON file (We have already imported that and stored in postAPIRequest).
    ...
    expect(jsonPOSTAPIResponseBody.booking).toHaveProperty('firstname'); //we are asserting the response body to have property 'firstname'.
    expect(jsonPOSTAPIResponseBody.booking).toHaveProperty('lastname'); //We are asserting the response body to have property 'lastname'.
    expect(jsonPOSTAPIResponseBody.booking.bookingdates).toHaveProperty('checkin'); //We are asserting the response body to have property 'checkin'.)
    ...
    expect(jsonPOSTAPIResponseBody.booking.lastname).toBe('C'); //We are asserting the response body to have value 'Kumar' for the property 'lastname'.

    expect(jsonPOSTAPIResponseBody.booking.bookingdates.checkin).toBe('2018-01-01'); //We are asserting the response body to have value '2023-06-01' for the property 'checkin'.
