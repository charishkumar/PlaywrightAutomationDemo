...
import putAPIRequest from '../../test-data/api_requests/PUT_API_Request.json'; // we need to import PUT API Request.json file
...
test('Create PUT API Request using Playwright', async ({ request }) => {
    ...
    //posting request to get the token for the API request. We are using this token in the header of the PUT API request to authenticate the request.
    ...
    //update booking
    // we are sending PUT request to update the specified booking ID, we need to use proper endpoint, along with content-type and token along with the PUT request.
    ...
    //Getting response from the PUT Request in JSON format and storing into a variable.
    const putResponseBody = await putResponse.json();

    //validating PUT API Response status
    expect(putResponse.status()).toBe(200);
    expect(putResponse.statusText()).toBe('OK');

    //printing the PUT API Response body onto the console.
    console.log(`PUT API Response Body : ${JSON.stringify(putResponseBody, null, 2)}`);
})