...
import {faker} from '@faker-js/faker'; //we are importing faker module to generate random data for the post request body.

test.use({
    baseURL:process.env.BASE_API_URL,
})

test('POST API Request Test using Faker plugin in Playwright', async ({ request }) => {
    ...
    //generating random data using faker and assigning it to a variable values. We are using faker.name.firstName(), faker.name.lastName(), faker.number.int({min: 1000, max: 5000}), and faker.lorem.word() methods to generate random data.
    ...
    const values = [firstName, lastName, totalPrice, additionalNeeds]; //array of values to replace the placeholders in the template.
