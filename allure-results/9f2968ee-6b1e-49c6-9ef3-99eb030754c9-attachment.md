...
> 87 |    const putResponseBody = await putResponse.json();
     |                            ^ SyntaxError: Unexpected token 'N', "Not Found" is not valid JSON
  88 | 
  89 |     expect (putResponse.status).toBe(200);
  90 |     expect (putResponse.statusText).toBe('OK');
  91 | 
  92 |    console.log(`PUT API Response Body : ${JSON.stringify(putResponseBody, null, 2)}`);
  93 | 
  94 | 
  95 | })
```