...
```ts
  ...
  92  |    });
  93  |    
  94  |      //console.log(putResponse.text())
  95  |    
> 96  |    const putResponseBody = await putResponse.json();
      |                            ^ SyntaxError: Unexpected token 'M', "Method Not Allowed" is not valid JSON
  97  | 
  98  |     expect (putResponse.status).toBe(200);
  99  |     expect (putResponse.statusText).toBe('OK');
  100 | 
  101 |    console.log(`PUT API Response Body : ${JSON.stringify(putResponseBody, null, 2)}`);
  102 | 
  103 | 
  104 | })
```