...
Locator: getByText('Orange').nth(1)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Orange').nth(1)
...
```ts
  ...
  8  |     await page.routeFromHAR('./har/fruits.har', {url: '*/**/api/v1/fruits', update:true});
  9  | 
  ...
  17 |     await (expect(page.getByText('Apple').nth(1))).toBeVisible();
> 18 |     await (expect(page.getByText('Orange').nth(1))).toBeVisible();
     |                                                     ^ Error: expect(locator).toBeVisible() failed
  19 |     await (expect(page.getByText('Banana').nth(1))).toBeVisible();
  20 |     await (expect(page.getByText('Strawberry').nth(1))).toBeVisible();
  21 |     await (expect(page.getByText('Grapes'))).toBeVisible();
  22 | 
  23 | });
```